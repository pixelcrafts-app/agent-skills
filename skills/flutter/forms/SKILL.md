---
name: forms
description: Apply when building forms on mobile — validation, field types, keyboard handling, error display, submission patterns. Auto-invoke when writing form screens.
---
...
- Use to show **format example**, not the label — show the expected format, not a restatement of the label
- Lower contrast than entered text but still meets minimum contrast requirements
...
- First field gets `autofocus: true` only when the screen's primary purpose is the form (login, search, compose). Never autofocus on screens where the form is a secondary action.
- Multi-field forms: declare `FocusNode`s at the form level, pass to each field
- `FocusNode` lifecycle (create/dispose): create in `initState`, dispose in `dispose` — never leak a FocusNode
...
- **Specific**: name the exact issue ("Email must contain @", not "Invalid email")
- **Actionable**: state what to do ("Password must include a number", not "Password invalid")
- **Non-blaming**: describe the condition, not the user's failure
- **No jargon**: plain language, no regex or technical terminology
...
- Submit button is disabled until all required fields are valid. This prevents submission errors and gives users a clear signal of what's missing.
