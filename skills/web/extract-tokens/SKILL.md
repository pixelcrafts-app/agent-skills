...
- A Figma connector/tool-server is not part of this pack — check the project's settings for any Figma integration (e.g., a configured skill or tool server)
...
Source of truth for the craft audit. Generated YYYY-MM-DD by the extract-tokens skill from <source>.
...
After writing, other web-standards skills (`craft-guide`, `premium-check`) consume this file when present.
...
- **Figma parity requires a connector/tool-server** — without one, the user must export. Not auto-pulled.
