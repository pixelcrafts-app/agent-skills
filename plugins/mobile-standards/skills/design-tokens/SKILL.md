...
When applying these rules to a stack not listed (Compose Multiplatform Desktop, Flutter Embedder, NativeScript, etc.), add an adapter section with the same generic-pattern → framework-match table shape. The rule content above the adapter section never changes — only the match patterns vary.
