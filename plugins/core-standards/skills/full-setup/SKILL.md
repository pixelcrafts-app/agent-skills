...
requires:
  - craft-config   # craft.json schema
...
For each detected stack, select the corresponding agent-skills pack:
| Detected | Pack |
|---|---|
...
4. After every file write: run the project's fast tests for the changed file; fix failures before writing the next file.
...
- If `CLAUDE.md` exists: append a `## agent-skills pipeline` section; do not replace the file
