...
Run when all entries in `.claude/progress.md` show status `contract-tests-passing`.
...
CONTEXT — locked contracts (paste full content):
  .claude/contracts/auth.contract.md
  ---
  <paste content>
  ---
  .claude/contracts/user.contract.md
  ---
  <paste content>
  ---

CONTEXT — acceptance tests (paste full content):
  .claude/acceptance-tests.md
  ---
  <paste content>
  ---

CONTEXT — implementation status from progress.md:
...
SCOPE
  ...
  Out: .claude/contracts/ (do not modify)
...
TASK
  ...
  6. Update .claude/progress.md with final status

OUTPUT
  Write to .claude/progress.md before returning.
...
On the 5th failed attempt, stop and write to `.claude/progress.md`:
```
integration: BLOCKED
attempts: 5
