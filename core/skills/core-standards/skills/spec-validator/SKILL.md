...
Before any implementation begins. If `.claude/spec.md` does not exist or its status is not `locked`, run this skill first.
...
Location: `.claude/acceptance-tests.md`
...
Every acceptance criterion in `spec.md` must have at least one test entry here.
...
4. No agent may modify a locked `acceptance-tests.md`
...
- Begin implementation before `spec.md` status is `locked`
- Write acceptance tests that match the implementation rather than the spec criteria
- Modify `acceptance-tests.md` after it is locked
