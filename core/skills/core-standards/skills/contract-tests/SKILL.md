...
- `.claude/contracts/<domain>.contract.md` — the locked contract
- `.claude/acceptance-tests.md` — for awareness of end-to-end criteria
...
2. Add file entries to `.claude/contracts/LOCKED`:
   ```
   tests/contracts/auth.contract.test.ts: <sha256>
   tests/contracts/user.contract.test.ts: <sha256>
   ```
3. The `protect-files.sh` hook treats locked contract test files as protected
...
- Fix loop runs contract tests on every write — exit 2 from post-test hook if failing
